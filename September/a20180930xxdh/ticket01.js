...
(function(){
    ...
    //console.log(dtNow);
    if(dtNow >= '2018-09-15 21:30:00') {
       window.location.href = "//mgc.qq.com/cp/a20180915ychxm/index.htm";
        end=1;
    }
    //LimitInfo();
...
//////////////////////////////////////////////////////////////////////////////////////////
function LimitInfo(){
    var sUrl="//apps.game.qq.com/cgi-bin/daoju/v3/api/hs/plug_ticket/tuan_goods_limit.cgi?_app_id=1004&_output_fmt=2&_biz_code=mgc&_act_id=60&propid="+ticketType+Math.random();
...
function BuyTicket(){
    ...
    var sUrl = "//apps.game.qq.com/cgi-bin/daoju/v3/hs/i_buy.cgi?_app_id=1004&_plug_id=9200&_biz_code=mgc&_act_id=60&propid="+ticketType+"&buynum="+$('#TicketNum').val()+"&areaid="+areaId+"&r="+Math.random()+"&roleid="+LoginManager.getUserUin()+"&getUid="+$('#TicketPresent').val()+"&rolename=";
    console.log(sUrl);
        ...
        if(i_buy.result!=0)
        {
            if(i_buy.result==-6360&&typeof(i_buy.serial) !="undefined")
            {
                if(confirm("您当前有一笔未支付订单，金额是"+(i_buy.price/10)+"Q点，是否支付该订单？")) {
                    price=i_buy.price;
                    var sUrl="//apps.game.qq.com/cgi-bin/daoju/v3/pay/i_pay.cgi?serial="+i_buy.serial+"&_app_id=1004&paytype=1&apptype=4&_output_fmt=2&r="+Math.random();h
            ...
            {
                alert('票已售完，如果您有尚未支付的订单，请前往<a href="http://daoju.qq.com/mall/myticket.shtml" target="_blank">支付</a>吧！');
            } else if(i_buy.result==-6107) {//未创建角色
              if(areaId == 888) {
                alert("抱歉，赠送QQ尚未在网页梦工厂创建角色！");
                 //var ss = '还差一步：请在 <a href="http://mgc.qq.com">http://mgc.qq.com</a> 页面登录QQ，再回来继续购票哦~';
              } else {
            ...
            price=i_buy.price;
            var sUrl="//apps.game.qq.com/cgi-bin/daoju/v3/pay/i_pay.cgi?serial="+i_buy.serial+"&_app_id=1004&paytype=1&apptype=4&_output_fmt=2&r="+Math.random();
...
function OrderSucc(){
    if(i_pay.result==1002)
    {
        alert('登录失效，请于页面注销登录后重新登录页面！');
        return;
    }
    else if(i_pay.result==0)
    {
            var sUrl = i_pay.urlParams + "";
             sUrl = sUrl.replace(/&amp;/g, '&');
             var isSanBox = i_pay.sandbox==1;
             fusion2.dialog.buy ({ 
              ...
              //  sandbox值为布尔型。true：使用； false或不指定：不使用。 
                ...
                onSuccess : function (opt) { 
                    ...
                    //alert('恭喜您，购买成功，请前往<a href="http://x5.qq.com/act/a20150720yixia/index.htm#box_04" target="_blank">抽奖</a>吧！');
                    //clearCookie();
                    //LoginedUserInfo();
...
///////////////////////////////////////////////////////////////////////////////////////
/*  |xGv00|1f88721245fdc629b5c008c10f96d7d */